...
export default function Home() {
  ...
  if (!storedList) {
    const list = getRandomMerchant(merchantsList, 3);
...
const selectReward = (selected:number) => {
  ...
  return (
<div className="font-sans flex flex-col items-center justify-center w-full min-h-screen  p-4">
  ...
  {didSelect ?<> <SelectedReward /> </> :<><main className="flex flex-col md:flex-row gap-8 items-center md:items-start justify-center w-full max-w-6xl">
    {/* Left section */}
    <div className="flex flex-col w-full md:w-3/5 lg:w-2/5 bg-gray-200 rounded-md shadow-md overflow-y-auto max-h-screen">
      {/* Image */}
    ...
    >
      <div className="row-span-3 w-24 px-4"> <img
            ...
            /></div>
      <div className="col-span-2 "><span>{merchant.type}</span></div>
      <div className="col-span-2 font-bold text-md text-black"><span>{merchant.code}</span></div>
      <div className="col-span-2 truncate text-xs"><span>{merchant.voucherValue} {merchant.voucherName}</span></div>
    </div>
  ))}
      </div>
    </div>
...
function SelectedReward(){
  const router = useRouter();
    ...
    const goReward = () => {
        ...
        router.push('/brand/content/selected');
    }
...
const gcs = [
  ...
  {
    img: 'https://1000logos.net/wp-content/uploads/2023/04/Starbucks-logo-500x281.png',
    type: 'Starbucks',
    id: '3',
    code: 'STAR200',
  ...
  {
    img: 'https://1000logos.net/wp-content/uploads/2023/04/Dunkin-Donuts-logo-500x209.png',
    type: "Dunkin'",
    id: '5',
    code: 'DUNKIN80',
    name: 'Dunkin’ Donuts',
